 ...
 */
public abstract class VertxHttpHandler<C extends ConnectionBase> extends
		VertxHandler<C> {
	...
	@Override
	protected void channelRead(final C connection,
			final DefaultContext context, final ChannelHandlerContext chctx,
		...
		if (connection != null) {
			...
			// We need to do this since it's possible the server is being used
		...
		if (msg instanceof HttpContent) {
			...
			if (buf != Unpooled.EMPTY_BUFFER && buf.isDirect()) {
				...
				if (msg instanceof LastHttpContent) {
					...
					return new AssembledLastHttpContent(newBuf,
							last.trailingHeaders(), last.getDecoderResult());
				} else {
			...
			if (msg instanceof BinaryWebSocketFrame) {
				...
				throw new IllegalStateException("Unsupported websocket msg "
						+ msg);
			}
	...
	public void write(ChannelHandlerContext ctx, Object msg,
		...
		if (msg instanceof WebSocketFrameInternal) {
			...
			case CLOSE:
				...
				break;
			case CONTINUATION:
				msg = new ContinuationWebSocketFrame(frame.isFinalFrame(), 0,
						buf);
				break;
			case PONG:
	...
	protected abstract void doMessageReceived(C connection,
